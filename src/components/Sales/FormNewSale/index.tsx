...
const FormNewSale = () => {
  ...
  const completeItemFields = (
    ...
    if (fields[index].code !== codeProduct) {
      const productToFillFields = productsDatabase.filter(
  ...
  const calculateQty = (
    ...
    let newCost: any =
      parseFloat(cost.replace(/[^\d,]/g, "").replace(",", ".")) * qty;
    ...
    let newMargin: any =
      (parseFloat(margin.replace(/[^\d,]/g, "").replace(",", ".")) -
        disccount) *
      qty;
    ...
    <Modal>
      <FormAdd onSubmit={handleSubmit(submit)} noValidate>
        ...
        <div className="DivButtonsReg">
          ...
          <button onClick={setModalAddSale} className="buttonCancelReg">
            Cancelar
          </button>
        </div>
      </FormAdd>
